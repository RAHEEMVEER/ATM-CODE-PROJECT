    ...
    if (userInpur2.option === "WITHDRAW") {
        let userInput3 = await inquirer.prompt([
            {
                name: "suggestions",
                type: "list",
                message: "YOU HAVE TWO OPTIONS TO WITHDRAW YOUR AMOUNT CHOICE IS YOURS:",
        ...
        if (userInput3.suggestions === "ATM SUGGESTED AMOUNT") {
            ...
            if (myBalance >= suggestionsAmount.atmSuggest) {
                console.log(chalk.yellow(`YOUR REMANING AMOUNT IS. ${(myBalance -=
                    suggestionsAmount.atmSuggest)}`));
            }
            else {
                console.log(chalk.red(`YOU DON'T HAVE MONEY TO WITHDRAW. YOUR CURRENT BALANCE IS: ${myBalance}`));
            }
        }
